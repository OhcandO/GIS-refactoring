...
function buildpopup(type, content) {
	...
	if (type === "instrFlow") {
		...
		/*헤더 부분 (DESC) */
		let headerTitle = content.meterNm.split(' ');
		
		desc = `<div class="flex_header_left" >${headerTitle[0]}</br>${headerTitle[1]}</div>
				<div class="flex_header_right" style="width:67%">
					<div style="border: 0.2px solid darkgray">${"관리번호:" + content.ftrIdn}</div>`;
		if (content.fcltyNm) {
			desc += `<div style="border: 0.2px solid darkgray">${content.fcltyNm} 블록</div>`;
		} else {
			desc += `<div style="border: 0.2px solid darkgray"> - </div>`;
		}
		const coltDt = content.coltDt ? content.coltDt : '-';
		desc += `<div style="border: 0.2px solid darkgray">갱신시간: ${coltDt}</div>`;
		desc += `</div>`;
		...
		<div class="qlty_uj">
	        <div class="flex_valu">
	            ...
	            <div class="valu_digit">${Number(nvl(valueCli, "0")) + " " + qualityUnit[0]}</div>
	        </div>
	        <div class="flex_valu">
	            <div class="valu_title">탁도</div>
	            <div class="valu_digit">${Number(nvl(valueTbi, "0")) + " " + qualityUnit[1]}</div>
	        </div>
	        <div class="flex_valu">
	            <div class="valu_title">전기전도도</div>
	            <div class="valu_digit">${Number(nvl(valueEc, "0")) + " " + qualityUnit[2]}</div>
	        </div>
	        <div class="flex_valu">
	            <div class="valu_title">pH</div>
	            <div class="valu_digit">${Number(nvl(valuePhi, "0")) + " " + qualityUnit[3]}</div>
	        </div>
	        <div class="flex_valu">
	            <div class="valu_title">수온</div>
	            <div class="valu_digit">${Number(nvl(valueTei, "0")) + " " + qualityUnit[4]}</div>
	        </div>
	        <div class="flex_valu">
	            <div class="valu_title">경보현황</div>
	            <div class="valu_digit">없음</div>
	        </div>
		</div>`;
	...
	let info = `<div id="${idw + content.ftrIdn}"
					name="${names}" style='cursor:grab; position: relative;' data-toggle="tooltip" data-placement="top"
...
let showOnce_FLW = function () {
	showOnce_FLW = function () {};
	document.querySelector('#popCheckFlw').dispatchEvent(new PointerEvent('click'));
}
//블럭 유량값
		       ...
		       success : function (result, textStatus, jqXHR) {
			       if (result.data.length > 0) {
				       for (const element of result.data) {
					       ...
					       if (mainMap.map.markerQLayer && mainMap.map.markerQLayer[element.ftrIdn]) {
						       valu = Number(nvl(valueTbi, "0")) + qualityUnit[0] + "<br><br><br>";
						       valu += Number(nvl(valueCli, "0")) + qualityUnit[1] + "<br><br><br>";
						       valu += Number(nvl(valuePhi, "0")) + qualityUnit[2] + "<br><br><br>";
						       valu += Number(nvl(valueEc, "0")) + qualityUnit[3] + "<br><br><br>";
						       valu += Number(nvl(valueTei, "0")) + qualityUnit[4];
						       $("#qlt" + element.ftrIdn + "> li").eq(1).html(valu);
...
let showOnce_QLT = function () {
	showOnce_QLT = function () {};
	document.querySelector('#popCheckQlt').dispatchEvent(new PointerEvent('click'));
}
/**
...
function fn_getWaterDrain() {
	...
	// 최초 오버레이 팝업이 생성되었을 때, 지도 위에 발생시킴
	
}

let showOnce_DRN = function () {
	showOnce_DRN = function () {};
	document.querySelector('#popCheckDrn').dispatchEvent(new PointerEvent('click'));
}
/**
