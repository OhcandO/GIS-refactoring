...
class LayerTree {
    ...
    #KEY_BOOL_IS_LAYER = `isLayer`;
    #KEY_BOOL_VISIBLE = `visible`;
    ...
    convertLayerInfoToLayerCode(array, key, parentName) {
        let initialValue = {};
        for (let i = 0; i < array.length; i++) {
            ...
            if (item.childList && item.childList.length > 0) {
                let child = this.convertLayerInfoToLayerCode(
                    item.childList,
                    key,
                    item.name
                );
    ...
    createWrap(array, level) {
        ...
        array.forEach((layer) => {
            ...
            const isLayer = layer[this.#KEY_BOOL_VISIBLE] || "N";
            let hasChild = false;
    ...
    selectableTree(array) {
        for (let i = 0; i < array.length; i++) {
            let layer = array[i];
            let id = layer.id;
            let visible = layer[this.#KEY_BOOL_VISIBLE] || "N";
    ...
    getTreeIdArr(typeName, ftrIdn = "") {
        let codeObjArr = [].concat(
            Object.values(GisApp.layerCode).filter(
                (codeObj) =>
                    codeObj.typeName === typeName && codeObj.isLayer === "Y"
            )
        );
        if (typeName == "WTL_BLSM_AS" && ftrIdn) {
            codeObjArr = codeObjArr.filter(
                (codeObj) => codeObj.fullname == ftrIdn
            );
        }
    ...
    showLayerWithTypeName(typeName) {
        ...
        function getCodeObjArrFromTypeName(typeName) {
            let codeObjArr = Object.values(GisApp.layerCode).filter(
                (codeObj) =>
                    codeObj.typeName === typeName && codeObj.isLayer === "Y"
            );
            ...
            */
            ...
            minZoom = codeObjArr.reduce(
                (pre, cur) => (cur.minZoom > pre ? Number(cur.minZoom) : pre),
                10
            );
            return codeObjArr;
        }
        ...
        function isEveryLayerNodeSelected(treeIdArr) {
            return treeIdArr.every((nodeId) =>
                this.instance.is_selected(nodeId)
            );
        }
